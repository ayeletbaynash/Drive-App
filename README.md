...
Post File:	POST	/api/files	- Creates a new file with the given name and content. Requires JSON body.
...
Add Permission:  POST /api/files/:id/permissions  - Grants 'read' or 'write' permission to another user. 
...
*9.* Register a second user ('cat') -> Gets ID 1
...
*10.* Create a new file 'new_document.txt' inside folder ID 0 -> Gets ID 2
...
*11.* Grant 'read' permission to user ID 1 ('cat') on file ID 2
